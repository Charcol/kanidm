...
use crate::be::idl_sqlite::{
    IdlSqlite, IdlSqliteConfig, IdlSqliteReadTransaction, IdlSqliteTransaction,
    IdlSqliteWriteTransaction,
};

#[cfg(feature = "xlsx")]
use crate::be::idl_xlsx::{
    IdlXlsx, IdlXlsxConfig, IdlXlsxReadTransaction, IdlXlsxTransaction, IdlXlsxWriteTransaction,
};

#[derive(Debug, Clone)]
pub struct IdlArcConfig {
    pub cache_target: Option<usize>,
    #[cfg(not(feature = "xlsx"))]
    pub lower_config: IdlSqliteConfig,
    #[cfg(feature = "xlsx")]
    pub lower_config: IdlXlsxConfig,
}
...
pub struct IdlArcReadTransaction<'a> {
    #[cfg(not(feature = "xlsx"))]
    db: IdlSqliteReadTransaction,
    #[cfg(feature = "xlsx")]
    db: IdlXlsxReadTransaction,
...
pub struct IdlArcWriteTransaction<'a> {
    #[cfg(not(feature = "xlsx"))]
    db: IdlSqliteWriteTransaction,
    #[cfg(feature = "xlsx")]
    db: IdlXlsxWriteTransaction,
...
impl<'a> IdlArcTransaction for IdlArcReadTransaction<'a> {
    fn get_identry(
        &mut self,
        au: &mut AuditScope,
        idl: &IDL,
...
impl<'a> IdlArcTransaction for IdlArcWriteTransaction<'a> {
    fn get_identry(
        &mut self,
        au: &mut AuditScope,
        idl: &IDL,
...
impl IdlArc {
    pub fn new(audit: &mut AuditScope, config: IdlArcConfig) -> Result<Self, OperationError> {
        let IdlArcConfig {
            cache_target,
            lower_config,
        } = config;

        let cache_target = cache_target.unwrap_or(DEFAULT_CACHE_TARGET);

        #[cfg(not(feature = "xlsx"))]
        let db = IdlSqlite::new(audit, lower_config)?;

        #[cfg(feature = "xlsx")]
        let db = IdlXlsx::new(audit, lower_config)?;

        // How many threads did the BE choose?
        let pool_size = db.get_pool_size();
        ...
        let idl_cache = ARCache::new(
            cache_target * DEFAULT_IDL_CACHE_RATIO,
            pool_size as usize,
            DEFAULT_CACHE_RMISS,
            DEFAULT_CACHE_WMISS,
            false,
        ...
        let name_cache = ARCache::new(
            cache_target * DEFAULT_NAME_CACHE_RATIO,
            pool_size as usize,
            DEFAULT_CACHE_RMISS,
            DEFAULT_CACHE_WMISS,
            true,
    ...
    pub fn read(&self) -> IdlArcReadTransaction {
    ...
    pub fn write(&self) -> IdlArcWriteTransaction {
        // IMPORTANT! Always take entrycache FIRST
